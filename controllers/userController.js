      ...
      updateAction === "$pull"
        ...
        await Promise.all([
          User.findByIdAndUpdate(id, {
            [updateAction]: { followers: new mongoose.Types.ObjectId(req.user._id) }, // Đẩy ObjectId vào followers
          }),
          User.findByIdAndUpdate(req.user._id, {
            [updateAction]: { following:new mongoose.Types.ObjectId(id) }, // Đẩy ObjectId vào following
          }),
        ]);
  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        ...
        badWords: [...badWordsInName, ...badWordsInUsername, ...badWordsInBio],
      });
    }
...
export const getTop4Follow = async (req, res) => {
  try {
    const userId = req.user._id; // Lấy ID người dùng hiện tại từ middleware xác thực
    if (!userId) {
    ...
    // Lấy danh sách 4 người dùng hàng đầu chưa được theo dõi
    const users = await User.find({
      _id: { $nin: currentUser.following, $ne: userId }, // Loại bỏ những người đang theo dõi và chính người dùng hiện tại
    })
...
export const getUsersIAmFollowing = async (req, res) => {
  try {
    ...
    const user = await User.findById(userId)
    // Tìm những người mà bạn đang theo dõi (nghĩa là ID của họ có trong trường 'following' của bạn)
